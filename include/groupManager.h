...
struct listnode {
    ...
    static inline bool canFit(struct listnode *a, size_t off, size_t size) {  // 返回1, 说明块内还有容量,可追加写入
    ...
    static inline bool appendWrite(PMEMobjpool* pop, struct listnode* a,
      ...
      if (sizeof(T) > 1) {  //
        *(T*)((char*)a + off) = *buf;
        ...
        pmemobj_memcpy(pop, ((char*)a + off), buf, size,
...
public:
    ...
    inline PMEMobjpool *get_pop() { return pool->pop; }
