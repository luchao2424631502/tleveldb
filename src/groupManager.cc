...
PMEMoid Group::alloc_node() {
    ...
    int ret = pmemobj_alloc(pop, &new_node_oid, sizeof(listnode), 0,
...
GroupManager::GroupManager(DevDaxDevice *devptr):_device_ptr(devptr) {
    ...
    if (1) {
        ...
        for (int i=0; i < partition_num; i++) {
          std::string filepath = fsdax_dir +
                                 std::string(DEFAULT_PARTITION_NAME) +
                                 std::to_string(i);
          partitions_path.push_back(filepath);
          // 创建 分区内存池 文件
          partitions.push_back(
              new Group(new FsDaxDevice(filepath, DEFAULT_POOL_LAYOUT_NAME,
                                    partition_size),
                    i));
        }
    }
